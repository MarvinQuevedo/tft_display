...
const uint32_t TFT_SPI_HZ = 20000000UL;
...
static void drawTitle(uint8_t fx) {
  ...
  tft.setCursor(6, 4);
  tft.setTextColor(ST77XX_WHITE, effectTitleBgColor(fx));
  tft.print(FX_NAMES[fx]);
  tft.fillRect(DRAW_W, 0, RIGHT_GHOST_MARGIN_PX, 16, ST77XX_BLACK);
}
...
static void initEffect(uint8_t fx) {
  ...
  if (fx == FX_MATRIX) {
    ...
    tft.fillRect(0, 16, DRAW_W, SCREEN_H - 16, ST77XX_BLACK);
...
static void stepMatrix(uint32_t now) {
  ...
  for (uint8_t i = 0; i < steps; i++) {
    ...
    if (body >= 0 && body < matrixRows) {
      ...
      char ch = MATRIX_CHARS[random(0, (int)(sizeof(MATRIX_CHARS) - 1))];
      tft.setCursor(x, y);
      tft.setTextColor(matrixBaseColor[c], ST77XX_BLACK);
      tft.print(ch);
    }
    int16_t tail = head - matrixTrailLen[c];
    if (tail >= 0 && tail < matrixRows) {
      int16_t y = 16 + tail * CHAR_H;
      tft.fillRect(x, y, CHAR_W, CHAR_H, ST77XX_BLACK);
    }
...
static void stepHyperspace() {
  for (uint8_t i = 0; i < MAX_PARTICLES; i++) {
    tft.drawPixel(particlePrevX[i], particlePrevY[i], ST77XX_BLACK);
    ...
    particlePrevX[i] = particles[i].x;
    particlePrevY[i] = particles[i].y;
    tft.drawPixel(particles[i].x, particles[i].y, ST77XX_WHITE);
  }
}
...
static void stepTronGrid() {
  if (frameCount % 8 == 0) {
    ...
    tft.drawFastHLine(0, y - 1, DRAW_W, ST77XX_BLACK);
  }
}
...
static void stepTerminatorGlitch() {
  if (frameCount % 2 == 0) {
    ...
    if (random(0, 4) == 0) {
      tft.fillRect(0, y, DRAW_W, 1, ST77XX_BLACK);
    }
  }
  if (frameCount % 25 == 0) {
    tft.fillRect(0, 16, DRAW_W, SCREEN_H - 16, ST77XX_BLACK);
...
static void stepBatSignal() {
  if (frameCount % 4 == 0) {
    ...
    tft.setTextColor(ST77XX_BLACK, panelColor(255, 230, 120));
    tft.setTextSize(2);
...
static void stepPacChase() {
  ...
  if (frameCount % 2 == 0) {
    if (prevPacX > -500) {
      tft.fillRect(prevPacX - 16, y - 16, 32, 32, ST77XX_BLACK);
      tft.fillRect(prevGhostX - 16, y - 16, 32, 32, ST77XX_BLACK);
      for (int16_t i = 0; i < DRAW_W; i += 20) {
        int16_t dotX = i + 4;
    ...
    uint16_t mouthColor = ST77XX_BLACK;
    if ((frameCount / 4) % 2 == 0) {
    ...
    tft.fillCircle(ghostX + 6, y - 12, 6, panelColor(255, 0, 120));
    tft.fillCircle(ghostX - 5, y - 8, 2, ST77XX_WHITE);
    tft.fillCircle(ghostX + 5, y - 8, 2, ST77XX_WHITE);
    prevPacX = pacX;
    prevGhostX = ghostX;
  }
